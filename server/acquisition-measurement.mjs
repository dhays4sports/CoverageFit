...
export async function refreshOpportunityMeasurement(repo,opportunityId){
  ...
  const fiv=detail.possessionQuality,priority=detail.opportunityPriority;
  const [fivRow,priorityRow]=await Promise.all([
...
function exposureInput(value={}){
  const exposureDay=day(value.exposureDay||value.incurredOn),sourceFamily=sourceText(value.sourceFamily),campaignId=clean(value.campaignId,160),exposures=Number(value.exposures),basis=sourceText(value.basis);
  if(!exposureDay)fail(422,'exposure_day','Choose the exposure date.');
  if(!SOURCE_FAMILY_SET.has(sourceFamily))fail(422,'source_family','Choose a source family.');
  if(!Number.isInteger(exposures)||exposures<1||exposures>100000000)fail(422,'exposures','Enter a whole-number exposure count from 1 to 100,000,000.');
  const allowed=new Set(['first_party_visit','platform_report','outbound_delivered','direct_mail_households','event_attendance','manual_import','other']);
  if(!allowed.has(basis))fail(422,'exposure_basis','Choose how the exposure count was measured.');
  return {campaignId,sourceFamily,sourceKey:clean(value.sourceKey,120),campaignVariant:clean(value.campaignVariant,120),partnerId:clean(value.partnerId,120),batchId:clean(value.batchId,120),exposureDay,exposures,basis,note:clean(value.note,600),evidenceRef:clean(value.evidenceRef,240)};
}

function blankGroup(key,attribution={}){return {key,sourceFamily:attribution.source_family||attribution.sourceFamily||'other',sourceKey:attribution.source_key||attribution.sourceKey||'',campaignId:attribution.campaign_id||attribution.campaignId||'',campaignVariant:attribution.campaign_variant||attribution.campaignVariant||'',partnerId:attribution.partner_id||attribution.partnerId||'',batchId:attribution.batch_id||attribution.batchId||'',spendCents:0,exposures:0,opportunities:0,qualified:0,contacts:0,conversations:0,quoteable:0,quotesPrepared:0,recommendationsDelivered:0,closeAsked:0,bound:0,boundPremiumCents:0,premiumEvidenceGaps:0,producerMinutes:0,effortMeasuredOpportunities:0};}
...
export function acquisitionMeasurement(repo,env={}){
  ...
  return {
    ...
    async exposure(value,requestId){const input=exposureInput(value),prior=await repo.sql('SELECT * FROM cf_acq_exposure_rollups WHERE workspace_id=? AND request_id=?',workspace,requestId).first();if(prior){const same=prior.campaign_id===input.campaignId&&prior.source_family===input.sourceFamily&&prior.source_key===input.sourceKey&&prior.campaign_variant===input.campaignVariant&&prior.partner_id===input.partnerId&&prior.batch_id===input.batchId&&prior.exposure_day===input.exposureDay&&Number(prior.exposures)===input.exposures&&prior.basis===input.basis&&prior.note===input.note&&prior.evidence_ref===input.evidenceRef;if(!same)fail(409,'request_reused','That exposure save request was already used. Reload before saving a different count.');return input;}const id=`exposure_${requestId}`,at=stamp();await repo.sql(`INSERT INTO cf_acq_exposure_rollups(id,workspace_id,exposure_day,source_family,source_key,campaign_id,campaign_variant,partner_id,batch_id,exposures,basis,note,evidence_ref,request_id,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,id,workspace,input.exposureDay,input.sourceFamily,input.sourceKey,input.campaignId,input.campaignVariant,input.partnerId,input.batchId,input.exposures,input.basis,input.note,input.evidenceRef,requestId,at).run();return input;},
    ...
    async daily(fromIso,toIso){
      await this.reconcile(300);
      const row=await repo.sql(`SELECT
        COUNT(CASE WHEN opportunity_created_at>=? AND opportunity_created_at<? THEN 1 END) AS opportunities,
        COUNT(CASE WHEN qualified_possession_at>=? AND qualified_possession_at<? THEN 1 END) AS qualified,
        COUNT(CASE WHEN contact_made_at>=? AND contact_made_at<? THEN 1 END) AS contacts,
        COUNT(CASE WHEN meaningful_conversation_at>=? AND meaningful_conversation_at<? THEN 1 END) AS conversations,
        COUNT(CASE WHEN quoteable_at>=? AND quoteable_at<? THEN 1 END) AS quoteable,
        COUNT(CASE WHEN quote_prepared_at>=? AND quote_prepared_at<? THEN 1 END) AS quotes_prepared,
        COUNT(CASE WHEN recommendation_delivered_at>=? AND recommendation_delivered_at<? THEN 1 END) AS recommendations_delivered,
        COUNT(CASE WHEN close_asked_at>=? AND close_asked_at<? THEN 1 END) AS close_asked,
        COUNT(CASE WHEN bound_at>=? AND bound_at<? THEN 1 END) AS bound,
        COALESCE(SUM(CASE WHEN written_premium_recorded_at>=? AND written_premium_recorded_at<? THEN written_premium_cents ELSE 0 END),0) AS written_premium_cents
      FROM cf_acq_opportunity_measurements WHERE workspace_id=?`,
        fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,fromIso,toIso,workspace).first();
      return {
        from:fromIso,to:toIso,
        opportunities:Number(row?.opportunities||0),qualified:Number(row?.qualified||0),contacts:Number(row?.contacts||0),conversations:Number(row?.conversations||0),quoteable:Number(row?.quoteable||0),quotesPrepared:Number(row?.quotes_prepared||0),recommendationsDelivered:Number(row?.recommendations_delivered||0),closeAsked:Number(row?.close_asked||0),bound:Number(row?.bound||0),writtenPremiumCents:Number(row?.written_premium_cents||0),
        basis:'first recorded milestone transitions within the Pacific day; call attempts are not inferred'
      };
    },
    async summary(params=new URLSearchParams()){
      ...
      const rows=await repo.rows(`SELECT a.*,m.* FROM cf_acq_opportunity_attribution a JOIN cf_acq_opportunity_measurements m ON m.workspace_id=a.workspace_id AND m.opportunity_id=a.opportunity_id WHERE a.workspace_id=? AND m.opportunity_created_at>=? AND m.opportunity_created_at<=?`,workspace,fromIso,toIso),spend=await repo.rows('SELECT * FROM cf_acq_spend WHERE workspace_id=? AND incurred_on>=? AND incurred_on<=?',workspace,fromDay,toDay);let exposureRows=[],exposureSetup=true;try{exposureRows=await repo.rows('SELECT * FROM cf_acq_exposure_rollups WHERE workspace_id=? AND exposure_day>=? AND exposure_day<=?',workspace,fromDay,toDay);}catch(error){if(/no such table/i.test(String(error?.message||'')))exposureSetup=false;else throw error;}const groups=new Map(),familyGroups=new Map();
      ...
      for(const e of exposureRows){const campaignKey=e.campaign_id?`campaign:${e.campaign_id}`:`source:${e.source_family}:${e.source_key||'unclassified'}:${e.partner_id||''}:${e.batch_id||''}`,g=groups.get(campaignKey)||blankGroup(campaignKey,e),fkey=`family:${e.source_family}`,fg=familyGroups.get(fkey)||blankGroup(fkey,{source_family:e.source_family});g.exposures+=Number(e.exposures||0);fg.exposures+=Number(e.exposures||0);groups.set(campaignKey,g);familyGroups.set(fkey,fg);}
      const sorted=[...groups.values()].map(g=>finalize(g,rate)).sort((a,b)=>b.boundPremiumCents-a.boundPremiumCents||b.qualified-a.qualified||b.spendCents-a.spendCents),families=[...familyGroups.values()].map(g=>finalize(g,rate)).sort((a,b)=>b.boundPremiumCents-a.boundPremiumCents||b.qualified-a.qualified),total=finalize(sorted.reduce((acc,g)=>{for(const k of ['spendCents','exposures','opportunities','qualified','contacts','conversations','quoteable','quotesPrepared','recommendationsDelivered','closeAsked','bound','boundPremiumCents','premiumEvidenceGaps','producerMinutes','effortMeasuredOpportunities'])acc[k]+=g[k];return acc;},blankGroup('total',{sourceFamily:'all'})),rate);
      return {build:ACQ_BUILD,period:{days,from:fromIso,to:toIso},commissionRateConfigured:rate,totals:total,groups:sorted,families,warnings:[...(!exposureSetup?['Exposure denominator is not configured yet. Apply migration 0019 before using per-1,000-exposure metrics.']:[]),...(total.exposures===0?['No exposure counts are recorded in this period, so per-1,000-exposure metrics are blank.']:[]),...(total.effortCoveragePct==null||total.effortCoveragePct<60?['Producer-time evidence is still sparse. Time-efficiency metrics are directional until more actual minutes are recorded.']:[]),...(total.bound&&total.premiumEvidenceGaps?['Some bound opportunities lack verified term-premium evidence and are excluded from bound premium.']:[])]};
    }
  };
}
