import {efficiencySummary} from './pilot-efficiency.mjs';
import {measurementLedgers} from './pilot-reconciliation.mjs';
...
export function validateObservation(input,receivedAt,now=new Date().toISOString()){
 ...
 o.farmers_policy_count=input.farmers_policy_count??null;if(o.farmers_policy_count!==null&&(!Number.isInteger(o.farmers_policy_count)||o.farmers_policy_count<0||o.farmers_policy_count>1000||o.bind!==true))fail(422,'policy_count','Farmers count requires a recorded bind and nonnegative integer.');
...
export const pilotSummary=records=>efficiencySummary(records,PILOT_FLAGS);
export function districtPilot(repo,env={}){const w=repo.scope.workspace;
 ...
 return {
  ...
  async quickReview(v,rid){
   ...
   if(Object.hasOwn(v,'bound_premium'))o.bound_premium=v.bound_premium;if(Object.hasOwn(v,'farmers_policy_count'))o.farmers_policy_count=v.farmers_policy_count;
  ...
  async report(){const found=await repo.rows(`SELECT s.summary_json,SUM(e.minutes) effort_minutes,MAX(e.created_at) effort_last_at FROM cf_solo_sources s LEFT JOIN cf_opportunity_effort e ON e.workspace_id=s.workspace_id AND e.opportunity_id=s.opportunity_id AND e.occurred_at>=json_extract(s.summary_json,'$.received_at') WHERE s.workspace_id=? AND s.kind=? GROUP BY s.source_id ORDER BY s.updated_at,s.source_id LIMIT 10001`,w,PILOT_KIND);if(found.length>10000)fail(422,'pilot_size','Pilot export exceeds 10,000 records; narrow the approved pilot before continuing.');const records=[],ledgers=await measurementLedgers(repo,found.map(row=>parse(row.summary_json)));for(const row of found){const p=parse(row.summary_json);if(p.pilot_phase!=='NEW_LEAD')continue;const o=p.observation||{},eff={minutes:row.effort_minutes,last_at:row.effort_last_at};let minutes=eff?.minutes??(o.effort_complete&&o.zero_effort_confirmed?0:null);const ledger=ledgers.get(p.opportunity_id),reconciled=p.reconciliation?.ledger_fingerprint===ledger.fingerprint;let complete=!!(o.effort_complete&&minutes!==null&&(!eff?.last_at||eff.last_at<=o.reviewed_at)&&(!ledger.last_call_at||ledger.last_call_at<=o.reviewed_at));if(p.reconciliation){minutes=reconciled?p.reconciliation.producer_minutes:minutes;complete=reconciled&&minutes!==null;}records.push({...p,sms_linked:!!p.conversation_id,...Object.fromEntries(PILOT_FLAGS.map(k=>[k,o[k]??null])),...Object.fromEntries(PILOT_DATES.map(k=>[k,o[k]??null])),bound_premium:o.bound_premium??null,producer_minutes:minutes,effort_complete:complete,call_attempts:reconciled?p.reconciliation.call_attempts:(ledger.calls||null),calls_complete:!!(reconciled&&p.reconciliation.call_attempts!==null),call_sources:reconciled?p.reconciliation.call_sources:ledger.call_sources,talk_minutes:null,farmers_policy_count:o.farmers_policy_count??null,reconciliation_source:p.reconciliation?.source||null,reviewed_at:o.reviewed_at||null,final_status:o.final_status||'OPEN',future_bind_month:o.future_bind_month||null});}
