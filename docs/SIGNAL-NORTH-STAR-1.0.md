...
### Publication and external observation — 2026-09-24

Code publication verified: CoverageFit main `09eb23d29658b368f663d84192a457bbf63f0f92`; branch `signal-north-star-1.0` retained at the same code commit. 408 main and `signal-routes-1.0` both `78660e8b5614ba4252af3ba55f35a543ccae21a1`, normal fast-forward from the recorded prior main through the comprehensive candidate. Both rollback refs remain unchanged. This ledger update is a later documentation-only commit.

Public browser observation: the production `/signal-preview/home/` page appeared after the Git merge; its DOM includes shared config and the production route shell. Existing `/home/` retains its previous review experience. CoverageFit `/agent/sms-operations/` still renders the protected Decision Queue, CALL NOW and Future Bind controls. GitHub combined-status responses exposed no deployment checks or deployed SHA; exact deployment SHA is not attested.

The Home anonymous canary did not pass in this browser: the page remained at “Preparing your questions”; console reported the route registry was undefined and navigation to the registry resource returned `net::ERR_BLOCKED_BY_CLIENT`. This is a browser/client loading restriction, not evidence of a site-wide outage or a proven source defect. No security setting was bypassed. Verify in the operator's normal browser after configuration, then continue remote decision certification. No identity/contact submission, provider send or AgencyZoom mutation occurred.

Manual Cloudflare configuration remains unconfirmed. Actual Home/SMS flag values are unknown; keep both off until their checkpoints. No live acquisition route has been activated. Home durable producer receipt, approval-only provider-send canary, real traffic and empirical calibration remain pending. The mandate is not declared fully complete.
